...
void setup(void);
void loop(void);
int8_t ee_write(uint8_t location, uint8_t *data, uint8_t len);
int8_t ee_read(uint8_t location, uint8_t *data, uint8_t len);
void ee_dump(void);
...
#   error Please set I2C_CHANNEL to either 0 or 1
    ...
    /* Set GPIO12 to I2CM_SCL and GPIO13 to I2CM_SDA... */
    gpio_function(12, pad_i2cm_scl);
    gpio_pull(12, pad_pull_none);
    gpio_function(13, pad_i2cm_sda);
...
int8_t ee_read(uint8_t location, uint8_t *data, uint8_t len)
{
    return i2cm_read(EEPROM_ADDR, location, data, len);
}
