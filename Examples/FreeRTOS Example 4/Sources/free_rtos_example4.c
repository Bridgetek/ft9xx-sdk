...
#if (configUSE_TRACE_FACILITY == 1)
      char *ucQueueType = myQueue->ucQueueType == queueQUEUE_TYPE_BASE ? "Queue             " : myQueue->ucQueueType == queueQUEUE_TYPE_MUTEX            ? "Mutex             "
                                                                                            ...
                                                                                            : myQueue->ucQueueType == queueQUEUE_TYPE_RECURSIVE_MUTEX    ? "Recursive Mutex   "
                                                                                                                                                         : "Unknown";
#else
  ...
  for (int i = 0; i < MAX_TIMER; i++)
  {
    ...
    if (myTimer != NULL)
    {
      printf("0x%08x %s 0x%08x %06d %s %s\r\n",
             ...
             myTimer->pxCallbackFunction == TimerCallbackOneShot ? "TimerCallbackOneShot" : "TimerCallbackAutoReload");
    }
  }
  printf("\r\n");
}
...
void DisplayHeapInfo()
{
  ...
  {
    unsigned int size = ptr->pxNextFreeBlock > 0 ? (int)ptr->pxNextFreeBlock - (int)ptr : 0;
