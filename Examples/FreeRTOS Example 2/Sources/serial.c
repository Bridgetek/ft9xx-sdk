...
xCharsForTx = xQueueCreate( uxQueueLength + 1, ( unsigned portBASE_TYPE ) sizeof( signed char ) );
	  ...
	  )
	{
		portENTER_CRITICAL();
		{
			...
			gpio_function(53, pad_uart1_txd); /* UART0 TXD */
			gpio_function(52, pad_uart1_rxd); /* UART0 RXD */
			uart_open(UART1, 1,
					UART_DIVIDER_230400_BAUD, uart_data_bits_8, uart_parity_none,
					uart_stop_bits_1);
...
else if(uart_is_interrupted(UART1, uart_interrupt_tx))
{

	if( xQueueReceiveFromISR( xCharsForTx,( void * ) &cChar,  &xHigherPriorityTaskWoken ) == pdTRUE )
	{
		/* Send the next character queued for Tx. */
		uart_write(UART1, cChar);
	}
	else{
		uart_disable_interrupt(UART1, uart_interrupt_tx);
	}
