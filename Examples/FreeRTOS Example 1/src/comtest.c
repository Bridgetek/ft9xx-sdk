...
#define comSTACK_SIZE				( ( unsigned short ) 512 )
...
void vStartComTestTasks( unsigned portBASE_TYPE uxPriority)
{
	...
	xPort = xSerialPortInitMinimal( 0, uxBufferLength );
...
TickType_t xTimeToWait;
	...
	for( ;; )
	{
		...
		vSerialPutString( xPort, (const signed char *)pcMessageToExchange, strlen( pcMessageToExchange ) );
...
short sResyncRequired, sConsecutiveErrors, sLatchedError;
	...
	for( ;; )
	{
		...
		xGotChar = xSerialGetChar( xPort, (signed char *)&cRxedChar, xBlockTime );
		if( xGotChar == pdTRUE )
		{
