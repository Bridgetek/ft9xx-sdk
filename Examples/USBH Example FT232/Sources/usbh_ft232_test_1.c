    ...
    // in the out buffer.
      while (len--)
      {
        ...
        if (uart0BufferOut_avail > 0)
        {
          ...
          if (uart0BufferOut_wr_idx == RINGBUFFER_SIZE)
            uart0BufferOut_wr_idx = 0;
  ...
  if (start == 0)
  {
    ...
    CRITICAL_SECTION_BEGIN
      ...
      // Get data from the out buffer.
        ...
        if (uart0BufferOut_rd_idx == RINGBUFFER_SIZE)
          uart0BufferOut_rd_idx = 0;
    ...
    CRITICAL_SECTION_BEGIN
        if (uart0BufferIn_avail < RINGBUFFER_SIZE)
        {
          ...
          *data = uart0BufferIn_data[uart0BufferIn_rd_idx];
          ...
          if (uart0BufferIn_rd_idx == RINGBUFFER_SIZE)
            uart0BufferIn_rd_idx = 0;
...
void uart0ISR ()
{
  ...
  if (uart_is_interrupted (UART0, uart_interrupt_rx))
  {
    ...
    if (uart0BufferIn_avail > 0)
    {
      ...
      if (uart0BufferIn_wr_idx == RINGBUFFER_SIZE)
        uart0BufferIn_wr_idx = 0;
    }
  }
}
...
void ft232_uart_bridge (USBH_FT232_context *ctx)
{
  ...
  static uint8_t bufUart2Usb[RINGBUFFER_SIZE] =
    { 0 };
  static uint8_t bufUsb2Uart[RINGBUFFER_SIZE] =
    { 0 };
  size_t read_bytes;
  int8_t ds;
...
int main (int argc, char *argv[])
{
  ...
  timer_init (timer_select_a, 1000, timer_direction_down,
              timer_prescaler_select_on, timer_mode_continuous);
  timer_enable_interrupt (timer_select_a);
  timer_start (timer_select_a);
  interrupt_attach (interrupt_timers, (uint8_t) interrupt_timers, USBH_timer);
