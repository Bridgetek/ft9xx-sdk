...
#define assert(__e)                                                                                    \
  do                                                                                                   \
  {                                                                                                    \
    static CONST char local[] = "ASSERT at " __FILE__ "+" INTSTR(__LINE__) ": " STRINGIZE(__e) "\r\n"; \
    if (__e) break;                                                                                    \
    printf("%s", (CONST char *)local);                                                                 \
    while (1);                                                                                         \
  } while (0)
 ...
 *         URBs are sent by calling the USBDX_submit_urb method, which is
...
/** @struct USBDX_pipe
 *  @brief Struct to maintain the pipe related data. A pipe structure is for each endpoint.
 */
struct USBDX_pipe;
...
struct USBDX_pipe {
  ...
  uint8_t *buf_end;
  /** Application�s callback function registered with USBD will be called at the event when USBD
   *  engine is ready. */
  USBDX_callback on_usbd_ready;
  /** Application's callback function registered with USBD will be called at the event when USBD
   *  engine is underrunning. **/
  ...
  bool usbd_paused;
  /** Application pause itself when no more buffer can be processed, and waiting for USBD to give
   *  more data. USBD engine will call the on_usbd_ready() callback once app is paused, to resume
...
/** @brief Get next application URB pointer, but it may still been hold by USBD engine at this
 *         point. To be used in conjunction with urb_owned_by_app() to know the ownership */
static inline struct USBDX_urb *usbdx_get_app_urb(const struct USBDX_pipe* pp)
{
  return pp->app_urb;
}

/** @brief Application pause itself when no more buffer can be processed, and waiting for USBD to
 *         give more data. USBD engine will call the on_usbd_ready() callback once app is paused,
 *         to resume the application's process. **/
 ...
 *  @details Application provides a linear space buffer to USBD. An array of URBs is initialised by
 *           USBD, dividing the linear space into small chunks (512 bytes for HS, 64 bytes for FS).
 *           Pipe is initialised by USBD, pointing to its own�URBs. Each Endpoint is represented by
 *           a pipe structure.
 ...
 **/
bool USBDX_pipe_init(struct USBDX_pipe *pp, uint8_t id, uint8_t ep, struct USBDX_urb *urbs,
                     uint8_t *bufs, uint8_t urb_count);

/** @name Optional functions for registering event callbacks. */
//@{
/** @brief To register callback function for USBD READY event.
 *  @details Application shall call this API to register their callback function to get notified of
 *           when USBD is ready to process the URBs.
...
/** @brief To register callback function for USBD UNDERRUN event.
 *  @details Application shall call this API to register their callback function to get notified of
 *           when USBD is underrunning when no data from application.
 *  @param [in] pp       - Pipe for which the callback function is associated to.
 *  @param [in] callback - Callback function to register.
 */
 ...
 *  @details Application can choose to acquire URB even if it is still held by USBD.
 ...
 *  @details Application fills the urb and submits to USBD through this API call. When a URB is
 *           submitted without filling, this will make USBD to send a ZLP.
 *  @attention This function should be protected by critical section since it is called from
 ...
 *  @attention This function should be called to obtain the length before copying and submitting the
 *             URBs through usbd_sumit_urbs().
 ...
 *             application's context.
 *  @param [in] pp  - The pipe whose data to be transferred to/from USBD.
 *  @param [in] len - The length must be equal or smaller than the length gotten from
 *                    usbd_get_app_urbs().
 ...
 *  @param [in] pipe_bitfields - Bits corresponding to Endpoint URBDX_pipe for which
 *                               USBDX_pipe_process has to be called.
 */
