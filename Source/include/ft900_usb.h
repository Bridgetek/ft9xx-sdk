 ...
 *        Used as bRequest in USB_device_request and bDescriptorType in USB_device_descriptor.
 */
//@{
/// 9.4.5 Get Status
...
#define USB_MICROSOFT_WCID_STRING(A) \
  ...
  USB_MICROSOFT_WCID_STRING_SIGNATURE A, 0x00,
 ...
 *  @brief Structure representing a USB Device Request.
 *         USB Spec Table 9-2.
 **/
...
typedef struct PACK USB_WCID_feature_descriptor
{
  ...
  /** The function's sub-compatible ID. **/
...
{
  /** The length, in bytes, of the complete extended property (Header and Property sections)
   *  descriptor (146 bytes) **/
  uint32_t dwLength;
  /** The descriptor's version number, in binary coded decimal (BCD) format. **/
  uint16_t bcdVersion;
  /** An index that identifies the particular OS feature descriptor. **/
  ...
  /** Property value in unicode e.g.'{8FE6D4D7-49DD-41E7-9486-49AFC6BFE470}'. Double Null
   *  termination **/
 ...
 *  @details For use with USB_REQUEST_CODE_GET_DESCRIPTOR for string descriptors (LANGID is the
 *           wIndex value) from USB Language Identifiers (LANGIDs) PDF Page 4
 **/
//@{
...
typedef struct PACK USB_hub_descriptor
{
  ...
  // From this point on there are variable size fields depending on bNbrPorts. One bit per provided
  // for compatibility reasons with V1.0 hubs only. Minimum sizes of these are 1 byte.
