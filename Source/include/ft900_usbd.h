 ...
 *  @brief Enums used to select the test modes.For more information refer to Section 7.1.20 of
 ...
 *  @brief Callback declaration for Vendor, Class and optionally Standard USB requests.
 *  @param [in] req - USB request.
 *  @return USBD_OK if the request was handled successfully; any other return value (such as
 *          FT9XX_FAILED) causes the USB driver to stall the control endpoints.
 */
typedef int8_t (*USBD_request_callback)(USB_device_request *req);

/** @typedef USBD_descriptor_callback
 *  @brief Callback declaration for standard get descriptor requests to obtain descriptor data.
 *  @param [in] req    - USB request.
 ...
 *  @param [in] req USB request.
 *  @return USBD_OK if the request is valid; any other return value (such as FT9XX_FAILED) causes
 *          the USB driver to stall the control endpoints.
 */
 ...
 *  @brief Callback declaration for transaction completion on endpoint. The endpoint number is
 *         passed to the callback to allow the same function to handle multiple endpoints.
 */
typedef void (*USBD_ep_callback)(USBD_ENDPOINT_NUMBER ep_number);

/** @struct USBD_ctx
 *  @brief Struct containing callback functions for the USB upper layer driver and callback
 *         functions for USB suspend/resume and USB reset. Sets USBD configuration information.
 */
typedef struct USBD_ctx
{
   ...
   * customize the responses to standard requests. If it is
   ...
   *  This may only be in the range of 8, 16, 32 or 64 bytes. For high speed operation it must be
   *  64 bytes.\n
   *      0: 8 bytes.
   *      1: 16 bytes.
   *      2: 32 bytes.
  ...
  /** Deprecated: Callback function for a data transfer to or from the control endpoint. This
   *              feature has been deprecated in the v2.5.0 release.
   */
  void *ep0_cb;

  /** Device configuration section.
   *  High speed/full speed select. The device will only operate in high speed mode if selected by
   *  this option.\n
   *      0: Full speed only.
 ...
 *  @details Performs a software reset and initialises the USB hardware.\n
 *           The USBD_ctx contains function pointers to the protocol layer handling USB requests.
 *           Appropriate USB requests will be routed to the correct handler, whether that be
 *           Standard, Class or Vendor requests.  A device may not need a handler for Vendor or
 *           Class requests depending on the device configuration.\n
 *           Optional function pointers are also available for USB suspend and resume callbacks and
 *           bus resets issued by the host.\n
 ...
 *  @details Releases any resources associated with the USB driver and disables the hardware.
 */
void USBD_finalise(void);
//@}

/** @brief Attach USB hardware.
 *  @details Attaches the USB device to the USB host after a USB_detach call.
 */
void USBD_attach(void);

/** @brief Detach USB hardware.
 *  @details Detaches the USB device from the USB host. This will look like device disconnect to the
 *           host and it will act like the device is removed.
 */
 ...
 *  @details Checks the VBUS detect line for a host connected and proceed to allow the device to
 *           negotiate a connection to the host.
 ...
 *  @details Places the device in the ADDRESS state. The application has to respond with SETUP ACK
 *           or STALL.
 *           The default standard request handler will call this function, if the handler is
 *           overridden then the application must call this when this request is received.
 *  @param [in] req - USB request.
 *  @return USBD_OK on success.\n
 *          USBD_ERR_INVALID_PARAMETER if req is invalid.
 **/
 ...
 *  @details Places the device in the CONFIGURED state or puts it back into the ADDRESS state. The
 *           application has to respond with SETUP ACK or STALL.
 *           The default standard request handler will call this function, if the handler is
 *           overridden then the application must call this when this request is received.
 *  @todo Possible future enhancement may be a method of checking configuration value defined by
 *        application.
 *  @param [in] req - USB request.
 *  @return USBD_OK on success.\n
 *          USBD_ERR_INVALID_PARAMETER if req is invalid.
 ...
 *  @details Handles the DATA phase of a GET_CONFIGURATION request from host. The application has to
 *           respond with SETUP ACK or STALL. The default standard request handler will call this
 *           function, if the handler is overridden then the application must call this when this
 *           request is received.
 *  @return USBD_OK on success.\n
 *          USBD_ERR_INVALID_PARAMETER if req is invalid.
 **/
 ...
 *  @details Stalls the specified endpoint. The default standard request handler will call this
 *           function for a SET_FEATURE endpoint request.
 ...
 *  @details Clears a stall from the specified endpoint. The default standard request handler will
 *           call this function for a CLEAR_FEATURE endpoint request.
 *  @param [in] ep_number - USB endpoint number.
 *  @return USBD_OK                    - if successful
            USBD_ERR_NOT_CONFIGURED    - if the endpoint is not configured.
            USBD_ERR_INVALID_PARAMETER - if the endpoint number is not allowed.
...
//@{
/** @brief Get USB state.
 *  @details Returns the current state of the current USB device. Please refer to section 9.1 of the
 *           USB 2.0 spec for more information.
 *  @return Current state of the current USB device.
 **/
USBD_STATE USBD_get_state(void);

/** @brief Set USB state.
 *  @details Sets the current state of the current USB device. Please refer to section 9.1 of the
 *           USB 2.0 spec for more information.
 ...
 *  @details When USB related events like host resume and host reset are detected, PM irq will be
 *           received if it is enabled. The firmware needs to remove the SUSPEND from the PHY by
 ...
 *           There is a total of 4 kB of RAM for FT90x Rev B, 6KB for FT90x Rev C and 8KB for FT930.
 *           This total RAM is for all the IN and OUT endpoints. Therefore the total max packet for
 *           all IN endpoints and OUT endpoints must be less than this figure. If double buffering
 *           is employed for an endpoint then it will use twice the amount of RAM.
 ...
 *  @param [in] ep_cb     - Deprecated: Callback function for this endpoint. This function was
 *                          called when an event concerned with the endpoint has occurred. This was
 *                          used for receiving notification of transaction to or from the endpoint
 *                          heralding the availability of data (OUT endpoints) or the completion of
 *                          a transmission of data (IN endpoints). However, the
 ...
 *          USBD_ERR_RESOURCES         - if there is not enough endpoint RAM for the endpoint size
 *                                       requested.
 **/
 ...
 *  @details Return the maximum number of bytes which can be sent or received in a single USB packets for an endpoint.
 ...
 *  @details    USB IN or OUT request is implied from the settings of the endpoint passed as a
 *              parameter. The end-of-packet will not be sent when the data from the buffer
 *              parameter is sent. This will allow a follow-on USBD_transfer_ex call to either send
 *              more data (with the part parameter non-zero and a correct offset set) or an
 *              end-of-packet with part not set. This allows a USB data packet to a non-control
 *              endpoint to be formed from multiple calls with data from potentially different
 *              places.
 *  @param [in] ep_number - USB endpoint number.
 ...
 *                          For OUT transfers, the maximum number of bytes to read.
 ...
 *  @param [in] offset    - Offset (within the current packet) from where to continue for subsequent
 ...
 **/
int32_t USBD_transfer_ex(USBD_ENDPOINT_NUMBER ep_number, uint8_t *buffer, size_t length,
                         uint8_t part, size_t offset);

/** @brief Send packet normally.
 *  @details This will send the data passed immediately, copying it into the transmission buffer and
 *           sending it. If the length falls on a wMaxPacketSize boundary then a ZLP will be sent
 *           after the packet (this will not happen for interrupt endpoints).
 **/
#define USBD_TRANSFER_EX_PART_NORMAL 0

/** @brief Send packet normally.
 *  @details This will pass data to the transmission buffer but not transmit it unless the data
 *           falls on or passes a wMaxPacketSize boundary. If this happens a packet of
 *           wMaxPacketSize length will be sent and any remaining data passed to the transmission
 *           buffer. No ZLP will be sent under any circumstances. The last packet sent in the
 *           sequence therefore cannot be sent with this method.
 */
 ...
 *  @param [in] ep_number - USB endpoint number.
 *  @param [in] buffer    - Appropriately sized buffer for the transfer.
 *  @param [in] length    - For IN transfers, the number of bytes to be sent. For OUT transfers, the
 *                          maximum number of bytes to read.
 *  @return The number of bytes actually transferred.
 *          USBD_ERR_NOT_CONFIGURED    - if endpoint is not configured.
 *          USBD_ERR_INVALID_PARAMETER - if endpoint number not allowed.
 **/
int32_t USBD_transfer(USBD_ENDPOINT_NUMBER ep_number, uint8_t *buffer, size_t length);

/** @brief Transfer data to/from a USB control endpoint.
 *  @details Endpoint number is assumed to be zero.
 *  @param [in] dir           - Control endpoint data direction.
 *  @param [in] buffer        - Appropriately sized buffer for the transfer.
 *  @param [in] dataLength    - For IN transfers, the number of bytes to be sent.
 *                              For OUT transfers, the maximum number of bytes to read.
 *  @param [in] requestLength - The number of bytes requested by the host in the wLength field of
 *                              the SETUP packet.
 *  @return The number of bytes actually transferred.
 *          USBD_ERR_NOT_CONFIGURED - if endpoint is not configured.
 **/
int32_t USBD_transfer_ep0(USBD_ENDPOINT_DIR dir, uint8_t *buffer, size_t dataLength,
                          size_t requestLength);

/** @brief USB process (Deprecated)
 *  @details Previously continuously called by the user application or USB device thread. Checked
 *           for control endpoint transfer activity and invoked relevant callback. Managed suspend
 *           and resume states and power management
 *  @note This is not required now as the actions are performed during the interrupt handler.
 *  @return Non-zero if USB transaction has been processed.
 */
__attribute__ ((deprecated)) int8_t USBD_process(void);

/** @brief USB timer
 *  @details To be called every millisecond from an interrupt handler to provide timeout support for
 *           USB device transactions. This will check all pending transfers, decrement timeout
 *           values and expire any timed out transactions.
 */
 ...
 *  @details When device is initialized and no bus activity for certain time, this API can be called
 *           to put the USB device to suspend. This API takes the USBD state to USBD_STATE_SUSPENDED
 */
