...
typedef enum
{
  ...
  uart_interrupt_9th_bit        = 0x06, /**< 9th bit set interrupt in 9-bit mode and also Receiver error interrupt */
  ...
  uart_interrupt_xon_xoff       = 0x10, /**< In-band flow control Interrupt (in 16950 mode only)
                                             along with Special character Interrupt for 9-bit data
                                             mode and Special character Interrupt */
  uart_interrupt_rts_cts        = 0x20, /**< Out-of-band flow control Interrupt (in 16950 mode only) */
} uart_interrupt_t;
...
#define uart_get_9bit_data_address(dev)                                                \
  (((((dev->LSR_ICR_XON2 & MASK_UART_LSR_PE_RXDATA) >> BIT_UART_LSR_PE_RXDATA) << 8) | \
  dev->RHR_THR_DLL) & 0x1FF)

/* FUNCTION PROTOTYPES *************************************************************/

/** @brief UART Software reset
 *  @details Issues UART Soft reset, which resets all UART registers to default value except
 ...
 *
 *  For N samples = 4 the following baud rates can be obtained from
 ...
 *  @returns 1 when interrupted, 0 when not interrupted, -1 otherwise (invalid device).
 */
int8_t uart_is_interrupted(ft900_uart_regs_t *dev, uart_interrupt_t interrupt);

/** @brief Enable or disable RTS signal
 *  @param [in] dev    - The device to use
 *  @param [in] active - Non-zero to enable RTS (line high) zero to disable (line low),
 ...
 *  @param [in] dev    - The device to use
 *  @param [in] active - Non-zero to enable DTR (line high) zero to disable (line low)
 *                       -1 otherwise (invalid device).
 */
...
int8_t uart_set_trigger_level(ft900_uart_regs_t *dev,  uart_mode_t mode,
                              ...
                              uart_fifo_trigger_level_t txIntLevel, uint8_t FCH, uint8_t FCL);
 ...
 *  @details Configure and enable 9-bit mode for sending and receiving 9-bit address and data. Upto
 *           four addresses can be configured for 9-bit mode address detection. Before configuring
 *           9-bit address, UART has to be enabled for uart_mode_16950.
 *  @param [in] dev       - The device to use
 *  @param [in] address_1 - One among four addresses for address detection
 *  @param [in] address_2 - One among four addresses for address detection
 *  @param [in] address_3 - One among four addresses for address detection
 *  @param [in] address_4 - One among four addresses for address detection
 *  @warning If only 1 address has to be configured for address matching, then all four address
 *           parameters should be configured with the same address.
 *  @returns 0 if successful, -1 otherwise (invalid device).
 */
int8_t uart_configure_9bit_address(ft900_uart_regs_t *dev, uint16_t address_1, uint16_t address_2,
                                   uint16_t address_3, uint16_t address_4);
