...
/** @brief The maximum number of endpoints which can be used in the library. The default value is
 *         for a control endpoints and an additional 4 BULK, INTERRUPT or ISOCHRONOUS endpoints. The
 *         more endpoints defined, the larger the interrupt code in ISR_usbd() and the more memory
 *         required for the USB_ep structure.
 *         Always count the control endpoint. There are a maximum of 8 endpoints available.
 */
//@{
...
/** @brief Enable interrupts to asynchronously update status of the endpoints and common events.
 *  @details The ISR_usbd function will be called at interrupt level when enabled by a hardware
 *           interrupt. If this is disabled then the USBD_process function will call the ISR_usbd
 *           function after disabling all other interrupts.
 */
#define USBD_USE_INTERRUPTS

/** @brief Enable streaming commands to access endpoint FIFO registers.
 *  @details The streamin and streamout assembler commands will be used when accessing the FIFO
 *           registers for endpoints. This is faster than a program loop to perform reads and writes
 *           to the FIFOs.
 */
#define USBD_USE_STREAMS

/** @brief Enable checking of endpoint configurations.
 *  @details When an endpoint is accessed this will enable certain checks to make sure it is
 *           configured and the endpoint number is valid.
 *           When endpoints are created this will ensure that the total number of bytes available in
 *           the endpoint FIFO RAM is not exceeded and that the max packet size of the endpoint is
 *           allowable for the type of endpoint and device speed. In all cases the endpoint used
 *           must be within the valid range of endpoints by the definition USBD_MAX_ENDPOINT_COUNT.
 */
 ...
 *         This structure is used in all calls to transfer data between the Host and Device.
 **/
typedef struct USBD_endpoint
{
  ...
  /** IN or OUT endpoint. Not applicable for control endpoint.
      From DC_EP_CONTROL register, see Table 9.6 in AN_324 for Rev B devices or Table 8.6 in
      BRT_AN_020 for Rev C devices.\n
        0: EP direction OUT.\n
        1: EP direction IN.
   **/
  ...
  /** BULK, ISO, INT (or CTRL) endpoint.
      From DC_EP_CONTROL register, see Table 9.10 in AN_324 for Rev B devices or Table 8.10 in
      BRT_AN_020 for Rev C devices.\n
        0: disabled.\n
        1: BULK.\n
        2: INTERRUPT.\n
  ...
  /** Last Transaction Status for endpoint. NOT CURRENTLY USED.
      Control endpoints:
      From DC_EP0_STATUS register, see Table 9.7 in AN_324 for Rev B devices or Table 8.7 in
      BRT_AN_020 for Rev C devices.\n
      ...
      From DC_EP_STATUS register, see Table 9.11 in AN_324 for Rev B devices or Table 8.11 in
      BRT_AN_020 for Rev C devices.\n
      Bit 5: Data error.
   ...
   **/
  uint8_t ep_status;
  /** Callback function for data being available to read for OUT endpoints or data read by host for
   *  IN endpoints. */
  void *cb;
} USBD_endpoint;
 ...
 *  @brief This flag is set when Remote Wakeup Enable request comes through DEVICE_REMOTE_WAKEUP
 *         feature selector. USB spec 9.4.5.
 */
static uint8_t USBD_remote_wakeup_enable = 0;

/** @name Test mode selector values from TEST MODE feature request.
 *  @brief This variable is set with the test mode selector values when a feature request for
 *         TEST MODE comes. USB spec 9.4.9.
 */
static uint8_t USBD_testmode_selector = 0;

#ifdef USBD_SW_SPEED_DETECT
/** @name speed detect flag
 *  @brief Variable used to initiate the USB bus speed detection
 ...
 *  @brief Variable used to signal that a Link Power Management event has occurred.
 */
//UNUSED: static volatile uint8_t USBD_LPM_changed = 0;

/** @name Current USB configuration value.
 *  @brief Zero when the device is in the Address state but non- zero once configured. Must match a
 *         valid configuration in the configuration descriptor. Set by SET_CONFIGURATION request.
 */
static uint8_t USBD_configuration = 0;

/** @name Callbacks for USB device events.
 *  @brief Callback functions for events. Certain SETUP requests and state changes require handling
 *         by an application. Most standard requests can be handled by the library without
 *         additional code in an application. If the USBD_standard_req_cb callback is not defined
 *         then the library will handle standard requests but will call USBD_descriptor_req_cb to
 *         get descriptors.
 *         All class and vendor requests must by handled by the application.
 *         If a request is not supported then the callback must return a value other than USBD_OK.
 *         If this happens then the library code will STALL the request. The suspend and resume
 *         callbacks are activated when the host stops and restarts bus activity (SOFs)
 *         respectively.
 *         The reset callback is used when the host asserts a reset state on the USB or the device
 *         is removed from the USB.
 */
//{@
...
#ifdef USBD_ENDPOINT_CHECKS
      // Disable all endpoints during callback to make sure that there are no transactions carried
      // out. USBD hardware also disables endpoints and resets endpoint configuration registers when
      // a reset is received.
...
static inline void epif_process(void)
{
  ...
  if (epif & MASK_USBD_EPIF_EP0IRQ)
  {
    ...
    if (USBD_EP_SR_REG(USBD_EP_0) & MASK_USBD_EP0SR_SETUP)
    {
      ...
      {
        // STALL end
        USBD_EP_CR_REG(USBD_EP_0) = USBD_EP_CR_REG(USBD_EP_0) & (~MASK_USBD_EP0CR_SDSTL);
        // Clear STALL send.
 ...
 * @details Handle standard GET_FEATURE and SET_FEATURE requests from the host application.
 *          The bmRequestType value is parsed and the appropriate action or function is performed.
 * @param [in] req - USB_device_request structure containing the SETUP portion of the request from
 *                   the host.
 * @return status - FT51_OK if successful or FT51_FAILED if there is an error or the bmRequestType
 *                  is not supported.
 **/
static int8_t usbd_standard_req_set_clear_feature(USB_device_request *req)
{
                             ...
                             USB_BMREQUESTTYPE_RECIPIENT_ENDPOINT))
  {
    ...
    if (req->wValue == USB_FEATURE_ENDPOINT_HALT)
    {
      ...
      if (((state < USBD_STATE_CONFIGURED) && (ep_number == 0)) || (state >= USBD_STATE_CONFIGURED))
      {
        status = USBD_OK;
 ...
 * @details Handle standard GET_STATUS requests from the host application.
 *          The bmRequestType value is parsed and the appropriate action or function is performed.
 * @param[in] req - USB_device_request structure containing the SETUP portion of the request from
 *                  the host.
 * @return status - FT51_OK if successful or FT51_FAILED if there is an error or the bmRequestType
...
static int8_t usbd_standard_req_get_status(USB_device_request *req)
{
                                  ...
                                  USB_BMREQUESTTYPE_RECIPIENT_DEVICE))
  {
    ...
    dev_req.bmRequestType = USB_BMREQUESTTYPE_DIR_DEV_TO_HOST | USB_BMREQUESTTYPE_RECIPIENT_DEVICE;
    if (USBD_descriptor_req_cb)
    {
      USBD_descriptor_req_cb(&dev_req, (uint8_t **)&config, &len);
      if (config)
      {
        if ((config->bmAttributes & USB_CONFIG_BMATTRIBUTES_REMOTE_WAKEUP) &&
            (USBD_remote_wakeup_enable))
        {
 ...
 * @details Handle standard requests from the host application. The bRequest value is parsed and the
 *          appropriate action or function is performed. Additional values from the
 *          USB_device_request structure are decoded and provided to other handlers.
 * @param [in] req - USB_device_request structure containing the SETUP portion of the request from
 *                   the host.
 * @return status - FT51_OK if successful or FT51_FAILED if there is an error or the bRequest is not
 *                  supported.
 */
...
#ifdef USBD_USE_INTERRUPTS
    USBD_REG(epie) = (MASK_USBD_EPIE_EP0IE | ((1 << (USBD_MAX_ENDPOINT_COUNT)) - 2));
                               ...
                               size_t offset)
{
  ...
  CRITICAL_SECTION_BEGIN
  {
    ...
    if (length)
    {
      ...
      {
        __asm__ volatile("streamout.b %0,%1,%2" : :
                         "r"(data_reg), "r"(buffer), "r"(length) : "memory");
      }
      bytes_read = length;
    }
 ...
 * @details Read in a SETUP request token from the USB device and enact any callbacks for the
 *          standard, class and vendor request types.
 */
static void usbd_process_setup_packet(void)
{
...
static void determine_usb_speed(void)
{
  ...
  USBD_speed = (SYS->MSC0CFG & MASK_SYS_MSC0CFG_HIGH_SPED_MODE) ? USBD_SPEED_HIGH : USBD_SPEED_FULL;
#else  /* __FT930__ */
  /* Detection by SOF */
  ...
  USBD_REG(cmif) = MASK_USBD_CMIF_SOFIRQ;
  delayus(125 + 5);
  USBD_speed = (USBD_REG(cmif) & MASK_USBD_CMIF_SOFIRQ) ? USBD_SPEED_HIGH : USBD_SPEED_FULL;
#endif /* !__FT930__ */
#ifdef USBD_DEBUG_CONFIGURE
...
static bool usbd_wait_epx_in_ready(USBD_ENDPOINT_NUMBER   ep_number)
{
  ...
  while (USBD_EP_SR_REG(ep_number) & MASK_USBD_EPxSR_INPRDY)
  {
    if (!USBD_transfer_timeout)
    {
      ...
      /* Writing a '1' to this bit flushes the next packet to be transmitted from the Endpoint 1
       * IN FIFO. The FIFO pointer is reset and the INPRDY bit is cleared.
       */
...
int8_t USBD_create_endpoint(USBD_ENDPOINT_NUMBER ep_num, USBD_ENDPOINT_TYPE ep_type,
                            USBD_ENDPOINT_DIR ep_dir, USBD_ENDPOINT_SIZE ep_size,
                            USBD_ENDPOINT_DB ep_db, void *ep_cb)
{
...
#ifdef USBD_ENDPOINT_CHECKS
  {
    ...
    // EP number higher than maximum allowed.
    if (ep_num >= USBD_MAX_ENDPOINT_COUNT)
      return USBD_ERR_NOT_SUPPORTED;

    // EP Size too big
    if ((ep_type != USBD_EP_ISOC) && (ep_type != USBD_EP_INT) && (ep_size > USBD_EP_SIZE_512))
    {
      return USBD_ERR_INVALID_PARAMETER;
    }
...
void USBD_initialise(USBD_ctx *c)
{
  ...
  if (c)
  {
    ...
    /* Speed is later updated based on the speed detected on the bus when connected */
  }
...
{
  // USB spec 9.4.5: Remote Wakeup is disabled(reset to zero) when the device is reset.
  USBD_remote_wakeup_enable = 0;
}
...
void USBD_set_test_mode(USBD_TESTMODE_SELECT test_selector)
{
  ...
  case USBD_TEST_PACKET:
    ...
    // Fill the EP0 FIFO with Test pattern
    {
      ...
      } while ((reg & MASK_USBD_EP0SR_INPRDY) && (reg & (MASK_USBD_EP0SR_STALL)) == 0);
