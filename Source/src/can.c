 ...
 *  @param [in] dev        - A pointer to the device to use
 *  @param [in] mode       - The mode that this CAn device should be in (normal or listen)
 *  @param [in] timeconfig - The configuration struct that defines the timing of the device
 *  @returns 0 on a success, -1 otherwise
 */
int8_t can_init(ft900_can_regs_t *dev, can_mode_t mode,
...
/** @brief Close the CAN device for reading and writing
 *  @param [in] dev - A pointer to the device to use
 *  @returns 0 on a success, -1 otherwise
 */
...
int8_t can_write(ft900_can_regs_t *dev, const can_msg_t *msg)
{
  ...
  if (iRet == 0)
  {
    ...
    if (msg->type == can_type_standard)
    {
      ...
      {
        dev->CAN_TX_BUF = msg->data[1] << 0  | msg->data[2] << 8 |
    ...
    else /* Extended message */
    {
      ...
      {
        dev->CAN_TX_BUF = msg->data[3] << 0  | msg->data[4] << 8 |
                          msg->data[5] << 16 | msg->data[6] << 24;
      }
 ...
 *
 *  @param [in] dev - A pointer to the device to use
 *  @param [in] msg - The struct to pack the message into
 *
 *  @returns 0 on a success, -1 otherwise
 *
 *  @warning This command only works when the CAN device is open
...
int8_t can_enable_interrupt(ft900_can_regs_t *dev, can_interrupt_t interrupt)
{
  ...
  if (iRet == 0)
  {
    ...
    case can_interrupt_data_overrun:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_DATA_OVRN_EN;
      break;
    case can_interrupt_bus_error:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_BUS_ERR_EN;
      break;
    case can_interrupt_transmit:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_TX_EN;
      break;
    case can_interrupt_receive:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_RX_EN;
      break;
    case can_interrupt_error_passive:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_ERR_PSV_EN;
      break;
    case can_interrupt_error_warning:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_ERR_WRNG_EN;
      break;
    case can_interrupt_arbitration_lost:
      dev->CAN_INT_ENABLE |= MASK_CAN_INT_ENABLE_ARB_LOST_EN;
      break;
 ...
 *
 *  @param [in] dev       - A pointer to the device to use
 *  @param [in] filtmode  - The mode that the filters should be in (single or dual)
 *  @param [in] filternum - The number of filter to use. When in single mode, only 0 will work.
 *  @param [in] filter    - A pointer to the configuration to use
 *
 *  @returns 0 on a success, -1 otherwise
 *
 *  @warning This command only works when the CAN device is closed
 ...
 *  @see can_filter_t
 */
int8_t can_filter(ft900_can_regs_t *dev, can_filter_mode_t filtmode, uint8_t filternum,
