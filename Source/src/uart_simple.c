...
/* Constant to be written to LCR register to access 650 mode registers */
#define MODE_650            (0xBF)

/* TYPES ***************************************************************************/

/**
 * @struct lc_regs
 * @brief structure holding local copy of LCR and ACR registers
 */
typedef struct
{
  uint8_t acr_copy;
...
/**
 * @struct reg_copy_t
 * @brief structure holding local copy of LCR and ACR registers for different uart channels
 */
typedef struct
{
  lc_regs uart0;
...
static int8_t uart_write_index_register(ft900_uart_regs_t *dev, uint32_t reg, uint8_t val);
...
static inline uint8_t uart_spr_read(ft900_uart_regs_t *uart, uint8_t location)
{
  ...
  {
    lc_regs *local = uart_get_copy(uart);
    /* return the previous written value of acr */
    val = local->acr_copy;
  }
  else
  {
    /* A memory barrier is necessary to stop the compiler from optimizing away multiple writes to
       the LSR_ICR_XON2 when this function is inlined */
    uart->SPR_XOFF2 = location;
 ...
 *  @details To access 650 registers 0xBF has to be written into LCR register
 ...
 *  @details To access normal registers other than 650 registers, some value other than 0xBF has to
 *           be written into LCR register
 *  @param [in] dev - The device to use
 */
static inline void uart_disable_650mode(ft900_uart_regs_t *dev)
{
  /* uart_read_lcr() returns lcr value removing xBF mode */
  uart_write_lcr(dev, uart_read_lcr(dev));
}
 ...
 *  @details To access some of the registers last value written to LCR should not be 0xBF. This
 *           function returns last value written to LCR
 *
 *  @param [in] dev - The device to use
 *
...
static inline uint8_t uart_read_lcr(ft900_uart_regs_t *dev)
{
  ...
  {
    /* Writing 0xBF to LCR only sets DLAB bit, remaining unchanged */
    tempLCR = dev->LCR_RFL & ~MASK_UART_LCR_DLAB;
  }
  else
    tempLCR = dev->LCR_RFL;
...
static uint8_t uart_read_index_register(ft900_uart_regs_t *dev, int8_t reg)
{
  ...
  {
    uint8_t temp;
    /* Enable access, ACR[6] (ICR) <- '1' */
    uart_spr_write(dev, OFFSET_UART_SPR_ACR, val | MASK_UART_ACR_ICR);
    /* Get the value */
    temp = uart_spr_read(dev, reg);
    /* Disable access, ACR[6] (ICR) <- '0' */
    uart_spr_write(dev, OFFSET_UART_SPR_ACR, val & ~MASK_UART_ACR_ICR);
    val = temp;
  }
...
static inline uint8_t uart_read_asr(ft900_uart_regs_t *dev)
{
  ...
  /* ACR[7] = 1 for accessing ASE registers */
  uart_write_index_register(dev, OFFSET_UART_SPR_ACR, (acr | MASK_UART_ACR_ASE));
  val = dev->IER_DLH_ASR;
...
static inline int8_t uart_write_asr(ft900_uart_regs_t *dev, uint8_t data)
{
  ...
  /* only bits 0 and 1 are RW in ASR */
  if (data & ~(MASK_UART_ASR_TX_DIS | MASK_UART_ASR_REM_TX_DIS))
  {
    iRet = -1;
  }
 ...
 *
 *  @param [in] dev       - The device to use
 *  @param [in] prescaler - The value of the prescaler
 *  @param [in] divisor   - The value of the divisor
 *  @param [in] databits  - The number of data bits
 *  @param [in] parity    - The parity scheme
 *  @param [in] stop      - The number of stop bits
 *
 *  @warning 1.5 stop bits is only available in 5 bit mode
 *
 *  @returns 0 on success, -1 otherwise (invalid device).
 */
int8_t uart_open(ft900_uart_regs_t *dev, uint8_t prescaler, uint32_t divisor,
                 uart_data_bits_t databits, uart_parity_t parity, uart_stop_bits_t stop)
{
  int8_t iRet = 0;
  uint8_t LCR_RFL;
  uint8_t tempLCR = 0;
 ...
 *
 *  @param [in] target_baud - The baud rate to use
 *  @param [in] samples     - The number of samples the UART will take for a bit, the default for this is 4
 *  @param [in] f_perif     - Peripheral frequency, the default for this is 100,000,000
 *  @param [in] divisor     - A pointer to store the divisor
 *  @param [in] prescaler   - A pointer to store the prescaler, if this is NULL the prescaler will be set to 1
 *
 ...
 *  these Divisors and Pre-scalers:
 *
 ...
 */
int32_t uart_calculate_baud(uint32_t target_baud, uint8_t samples, uint32_t f_perif,
                            uint16_t *divisor, uint8_t *prescaler)
{
  uint32_t lBaud = 0;
  uint16_t wDivisor = 0;
  ...
  for (bPrescaler = 1; (bPrescaler < bPrescalerLoopEnd) && (lBaudErrorBestMatch != 0UL); ++bPrescaler)
  {
    ...
    /* log2(2^16) hops = 16 hops */
    wDivisor = wHopSize = (DIVISOR_MAX / 2) + 1;
 ...
 *
 *  @param [in] dev    The device to use
 *  @param [in] buffer A pointer to the array to send
 ...
 *
 *  @param [in] dev    - The device to use
 *  @param [in] buffer - A pointer to the data word to store into
 *
 ...
 *
 *  @param [in] dev    - The device to use
 *  @param [in] buffer - A pointer to the array of data words to store into
 *  @param [in] len    - The number of data words to read
 *
 *  @returns The number of bytes read or -1 otherwise
 */
...
int8_t uart_enable_interrupt(ft900_uart_regs_t *dev, uart_interrupt_t interrupt)
{
  ...
  if (iRet == 0)
  {
    ...
    case uart_interrupt_9th_bit:
      uart_write_index_register(dev, OFFSET_UART_SPR_NMR,
                                uart_read_index_register(dev, OFFSET_UART_SPR_NMR) | MASK_UART_NMR_9_INT_EN);
      break;
    ...
    /* This interrupt is multiplexed with 9-bit special character detection and special character
       detection */
 ...
 *  @param [in] dev - The device to use
 *  @returns enum of possible interrupt levels. Cast to values defined in enum uart_interrupt_t.
 */
  ...
  if (iRet == 0)
  {
    ...
    case uart_interrupt_9th_bit:
      if (((dev->ISR_FCR_EFR & 0x3F) == uart_interrupt_9th_bit) &&
          (((dev->LSR_ICR_XON2 & (MASK_UART_LSR_PE_RXDATA | MASK_UART_LSR_RHRERR)) == MASK_UART_LSR_PE_RXDATA)))
      {
        iRet = 1;
      }
      break;
 ...
 *  @details After the mode is selected all flow control and UART settings are reset. The uart_open
 *           function must be called again to re-initialise the UART.
 *  @param [in] dev  - The device to use
...
int8_t uart_mode(ft900_uart_regs_t *dev, uart_mode_t mode)
{
  ...
  if (iRet == 0)
  {
    ...
    case uart_mode_16450:
      ...
      /* No FIFOs in 16450 mode. */
      temp &= ~MASK_UART_FCR_EN;
    ...
    case uart_mode_16550:
      ...
      /* Reset Enhanced mode if set */
      dev->ISR_FCR_EFR &= ~MASK_UART_EFR_EM;
      if (temp == MODE_650)
        uart_disable_650mode(dev);
      temp = uart_read_index_register(dev, OFFSET_UART_SPR_RFC);
    ...
    case uart_mode_16950:
      ...
      dev->ISR_FCR_EFR |= (MASK_UART_FCR_EN);
      /* FIFO enable, Tx FIFO reset, Rx FIFO reset */
      // | MASK_UART_FCR_RCVR_RST | MASK_UART_FCR_XMIT_RST);
      break;
 ...
 *  @param [in] rxIntLevel - Receiver interrupt level
 *  @param [in] txIntLevel - Transmitter interrupt level
 *  @param [in] FCH        - Flow Control High
 *  @param [in] FCL        - Flow Control Low
 *
...
int8_t uart_set_trigger_level(ft900_uart_regs_t *dev, uart_mode_t mode,
                              uart_fifo_trigger_level_t rxIntLevel,
                              uart_fifo_trigger_level_t txIntLevel,
                              uint8_t FCH, uint8_t FCL)
{
  ...
  if (iRet == 0)
  {
    ...
    case uart_mode_16950:
      if ((dev == NULL) || (rxIntLevel > 127) || (txIntLevel > 127) || (FCH > 127) || (FCL > 127))
      {
        iRet = -1;
      }
      ...
      if (iRet == 0)
      {
        ...
        uart_write_index_register(dev, OFFSET_UART_SPR_ACR,
                                  uart_read_index_register(dev, OFFSET_UART_SPR_ACR) | MASK_UART_ACR_950TLE); // ACR_950TLE
      }
      break;
...
uint8_t uart_get_rx_fifo_level(ft900_uart_regs_t *dev)
{
  ...
  val = dev->LCR_RFL;
  /* revert access */
  uart_write_index_register(dev, OFFSET_UART_SPR_ACR, acr);
  return val;
}
 ...
 *  @param [in] dev  - The device to use
 *  @param [in] mode - flow control mode to enable.
 *  @param [in] xOn  - xOn character(only used in XON-XOFF flow control, ignored in other cases).
 *  @param [in] xOff - xOff character(only used in XON-XOFF flow control, ignored in other cases).
 *  @returns 0 if successful, -1 otherwise (invalid device).
 */
int8_t uart_set_flow_control(ft900_uart_regs_t *dev, uart_flow_t mode, uint8_t xOn, uint8_t xOff)
{
  ...
  case uart_flow_dtr_dsr:
    ...
    {
      uart_write_index_register(dev, OFFSET_UART_SPR_CKS, cks & ~MASK_UART_CKS_TCRG);
    }
    /*Enable DTR is inactive*/
    dev->MCR_XON1_TFL |= MASK_UART_MCR_DTR;
    /*set ACR[4] = 0 and ACR[3] = 1 [page 68]*/
    uart_write_index_register(dev, OFFSET_UART_SPR_ACR,
                                   (uart_read_index_register(dev, OFFSET_UART_SPR_ACR) & ~MASK_UART_ACR_DTR1) | MASK_UART_ACR_DTR0);
    break;
  ...
  case uart_flow_xon_xoff:
    ...
    if ((efr & MASK_UART_EFR_EM) == MASK_UART_EFR_EM)
    {
      ...
      efr = dev->ISR_FCR_EFR;
      /*In-band transmit: recognize XON1 as XON and XFF1 as XOFF*/
      efr = ((efr & ~MASK_UART_EFR_IBTFCM0) | MASK_UART_EFR_IBTFCM1);
      /*In-band receive: recognize XON1 as XON and XFF1 as XOFF*/
      efr = ((efr & ~MASK_UART_EFR_IBRFCM0) | MASK_UART_EFR_IBRFCM1);
      dev->ISR_FCR_EFR = efr;
      uart_disable_650mode(dev);
    }
    else
 ...
 *  @details Configure address for 9-bit mode address detection
 *  @param [in] dev       - The device to use
 *  @param [in] address_1 - One among 4 address for address detection
 *  @param [in] address_2 - One among 4 address for address detection
 *  @param [in] address_3 - One among 4 address for address detection
 *  @param [in] address_4 - One among 4 address for address detection
 *  @warning if 9th bit of the address is not set, then that Particular address will not be
 *           configured or disabled for address detection.
                                   ...
                                   uint16_t address_2, uint16_t address_3, uint16_t address_4)
{
  ...
  if (iRet == 0)
  {
    ...
    // 9bit mode address detection is available only in enhanced mode.
    if ((dev->ISR_FCR_EFR & MASK_UART_EFR_EM) == MASK_UART_EFR_EM)
    {
 ...
 *  @details Clears all bytes in the receiver FIFO and resets its counter logic to 0
 *  @param [in] dev - The device to use
...
int8_t uart_flush_rx_fifo(ft900_uart_regs_t *dev)
{
  ...
  {
    dev->ISR_FCR_EFR = uart_read_index_register(dev, OFFSET_UART_SPR_RFC) | MASK_UART_FCR_RCVR_RST;
  }
 ...
 *  @details Clears all bytes in the transmitter FIFO and resets its counter logic to 0
 *  @param [in] dev - The device to use
 *  @warning The shift register is not cleared
 *  @returns 0 if successful, -1 otherwise (invalid device).
 */
int8_t uart_flush_tx_fifo(ft900_uart_regs_t *dev)
{
  ...
  {
    dev->ISR_FCR_EFR = uart_read_index_register(dev, OFFSET_UART_SPR_RFC) | MASK_UART_FCR_XMIT_RST;
  }
 ...
 *  @details Send 9-bit address characters from UART. before sending 9-bit address, UART has to
 *           enabled for 9-bit mode.
 *  @param [in] dev     - The device to use
 *  @param [in] address - 9 bit address to send out
 *  @returns 0 if successful, -1 otherwise (invalid device).
 */
