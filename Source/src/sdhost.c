...
static void sdhost_error_recovery(void)
{
  ...
  if (errIntStatus & 0x07FF)
  {
    ...
    if (errIntStatus & 0x000F)
    {
      ...
      {
        write_sdhost_reg(SW_RESET_CMD_LINE, SDH_SW_RST);
        /* Note we need a delay here else the write and read are too fast for SD Controller to react
           when compiled with Os flag
           See redmine bug 402 - http://glared1.ftdi.local/issues/402 */
        delayus(1);
      } while ((read_sdhost_reg(SDH_SW_RST) & SW_RESET_CMD_LINE) && (--timeout != 0));
      DBG_TIMEOUT()
      if (timeout == 0)
      {
        return;
    ...
    if (errIntStatus & 0x0070)
    {
      ...
      do
      {
        ...
        /* Note we need a delay here else the write and read are too fast for SD Controller to react
           ...
           See redmine bug 402 - http://glared1.ftdi.local/issues/402 */
        delayus(1);
      } while ((read_sdhost_reg(SDH_SW_RST) & SW_RESET_DATA_LINE) && (--timeout != 0));
      DBG_TIMEOUT()
      if (timeout == 0)
      {
        return;
    ...
    /* Enable the Error Interrupt Signal */
    write_sdhost_reg(read_sdhost_reg(SDH_ERR_INT_SGNL_ENABLE) | errIntStatus,
 ...
 *
 *  Access to the SD host peripheral registers can only be per performed on a 32 bit basis.
 *  read_sdhost_rec() takes care of any masking and shifting when accessing a register
 *  less than 32 bits.
 *
 *  @param [in] addr - Register ID.
 ...
 *  All this is absolutely necessary as we can only access the SDhost hardware in 32 bit chunks.
...
static void write_sdhost_reg(uint32_t val, uint32_t addr)
{
  ...
  case SDH_HST_CNTL_1:
    SDHOST->HOSTCTRL1_PWR_BLKGAP = val | (SDHOST->HOSTCTRL1_PWR_BLKGAP & 0xffffff00);
    break;

  case SDH_PWR_CNTL:
    SDHOST->HOSTCTRL1_PWR_BLKGAP = (val << 8) | (SDHOST->HOSTCTRL1_PWR_BLKGAP & 0xffff00ff);
    break;

  case SDH_CLK_CNTL:
    SDHOST->CLK_TIMEOUT_SWRESET = val | (SDHOST->CLK_TIMEOUT_SWRESET & 0xffff0000);
    break;

  case SDH_SW_RST:
    SDHOST->CLK_TIMEOUT_SWRESET = (val << 24) | (SDHOST->CLK_TIMEOUT_SWRESET & 0xffffff);
    break;

  case SDH_NRML_INT_STATUS:
    SDHOST->NORMINTSTATUS_ERRINTSTATUS = val | (SDHOST->NORMINTSTATUS_ERRINTSTATUS & 0xffff0000);
    break;

  case SDH_ERR_INT_STATUS:
    SDHOST->NORMINTSTATUS_ERRINTSTATUS = (val << 16) | (SDHOST->NORMINTSTATUS_ERRINTSTATUS & 0xffff);
    break;

  case SDH_NRML_INT_STATUS_ENABLE:
    SDHOST->NORMINTSTATUSEN_ERRINTSTATUSEN = val | (SDHOST->NORMINTSTATUSEN_ERRINTSTATUSEN & 0xffff0000);
    break;

  case SDH_ERR_INT_STATUS_ENABLE:
    SDHOST->NORMINTSTATUSEN_ERRINTSTATUSEN = (val << 16) | (SDHOST->NORMINTSTATUSEN_ERRINTSTATUSEN & 0xffff);
    break;

  case SDH_NRML_INT_SGNL_ENABLE:
    SDHOST->NORMINTSIGEN_ERRINTSIGEN = val | (SDHOST->NORMINTSIGEN_ERRINTSIGEN & 0xffff0000);
    break;

  case SDH_ERR_INT_SGNL_ENABLE:
    SDHOST->NORMINTSIGEN_ERRINTSIGEN = (val << 16) | (SDHOST->NORMINTSIGEN_ERRINTSIGEN & 0xffff);
    break;

  case SDH_HST_CNTL_2:
    SDHOST->AUTOCMD12_HOSTCTRL2 = (val << 16) | (SDHOST->AUTOCMD12_HOSTCTRL2 & 0xffff);
    break;
...
void sdhost_init(void)
{
   ...
   * vendor register 5 sets the card debouncing debug time
   * where 0 = 2^^9, 1 = 2^^10 ... 10 - 2^^24
   */
  write_sdhost_reg(0x0B, SDH_VNDR_5);

  /* Writing into the debounce register triggers a de-bounce sequence and you have to wait here till
     the debounce is done before you do anything else!
     See redmine bug 402 - http://glared1.ftdi.local/issues/402 */
  timeout = COMM_TIMEOUT_U32;
  /* wait for status to be debounced */
  while (((read_sdhost_reg(SDH_PRESENT_STATE) & PRESENT_STATE_SYS_CARD_STABLE) == 0)
      && (--timeout != 0));
  DBG_TIMEOUT()
  ...
  /* Configure CRC status wait cycle, pulse latch offset to 0x01 (pulse latch offset MUST be set
     to 0x01) */
  write_sdhost_reg(0x02000101, SDH_VNDR_0);
  write_sdhost_reg(0, SDH_VNDR_1);

  /* In the card identification mode, the maximum clock frequency is 400 kHz. Have to set the
     divider to reduce the clock speed */
  /* Enable internal clock */
  write_sdhost_reg(read_sdhost_reg(SDH_CLK_CNTL) | 0x0001, SDH_CLK_CNTL);

  /* Wait for the internal clock to be stable */
  timeout = COMM_TIMEOUT_U32;
  while ((!(read_sdhost_reg(SDH_CLK_CNTL) & CLK_CTRL_CLK_STABLE)) && (--timeout != 0))
    ;
  DBG_TIMEOUT()
  if (timeout == 0)
    return;
 ...
 *
 *  This is an internal function that extracts a response from the SD host peripheral in
 *  response to an issued command.
 *
 *  @param [in] response                 - A pointer to the array (R2) / variable (others) that will
 *                                         hold the response
 *  @param [in] response_type            - The response type
 *  @param [in] is_auto_command_response - Indicates whether the response is an auto CMD response
 *                                         (auto CMD12/CMD23)
 *  @returns SDHOST_STATUS
 */
static SDHOST_STATUS sdhost_get_response(uint32_t *response, sdhost_response_t response_type,
                                         bool is_auto_cmd_response)
{
 ...
 *  @param [in] cmd_index       - Command to generate (0-63)
 *  @param [in] cmd_type        - Command type (bus / application-specific)
 *  @param [in] sdio_special_op - The SDIO special operation for CMD52, assign NOT_IN_USE if the
 *                                operation is not one of the 3 special ones
 *  @param [in] resp_type_ptr   - Pointer to a variable to save the response type for the cmd
                                    ...
                                    uint8_t sdio_special_op, sdhost_response_t *resp_type_ptr)
{
  ...
  if (cmd_type == SDHOST_BUS_CMD)
  {
    ...
    /**
     * Refer to the SD Physical Layer Simplified Spec page 69 for the response type for each command
     */
    switch (cmd_index)
    {
  ...
  else if (cmd_type == SDHOST_APP_SPECIFIC_CMD)
  {
    ...
    switch (cmd_index)
    {
    /* Refer to the SD Physical Layer Simplified Spec page 74 for the response type for each
       command */
 ...
 *  @param [in] sdio_special_op - The SDIO special operation for CMD52, assign NOT_IN_USE if the
 *                                operation is not one of the 3 special ones
 *  @param [in] cmd_arg         - Command argument (32 bits)
 *  @param [in] response        - Pointer to the array (R2) / variable (others) that will hold the
 *                                response
 *  @returns A SD Host status
 */
static SDHOST_STATUS sdhost_send_command(uint8_t cmd_index, sdhost_cmd_t cmd_type,
                                         uint8_t sdio_special_op, uint32_t cmd_arg,
                                         uint32_t *response)
{
#ifdef SD_HOST_DEBUG
  printf("--------------------------------\r\n");
  ...
  cmd_reg_value = sdhost_generate_cmd(cmd_index, cmd_type, sdio_special_op, &response_type);
   ...
   */
  timeout = COMM_TIMEOUT_U32;
  while (((present_state = read_sdhost_reg(SDH_PRESENT_STATE)) & PRESENT_STATE_CMD_INHIBIT_C)
      && (--timeout != 0));
  DBG_TIMEOUT()
  if (timeout == 0)
  {
  ...
  {
    /**
     *  If the host driver does not issue an abort command, check cmd_inhibit_d in the present state
     *  register. Repeat this step until cmd_inhibit_d becomes 0.
     */
    if ((cmd_reg_value & CMD_TYPE_ABORT) != CMD_TYPE_ABORT)
    {
      timeout = COMM_TIMEOUT_U32;
      while ((read_sdhost_reg(SDH_PRESENT_STATE) & PRESENT_STATE_CMD_INHIBIT_D)
          && (--timeout != 0));
      DBG_TIMEOUT()
      if (timeout == 0)
      {
  ...
  timeout = COMM_TIMEOUT_U32;
  while (((read_sdhost_reg(SDH_NRML_INT_STATUS) & NORM_INT_STATUS_CMD_COMPLETE) == 0)
      && (--timeout != 0))
  {
    /**
  ...
  if (present_state & PRESENT_STATE_CMD_INHIBIT_D)
  {
    ...
    /* BUG: have a timeout so that won't block forever*/
    ...
    timeout = COMM_TIMEOUT_U32;
    while (((read_sdhost_reg(SDH_NRML_INT_STATUS) & NORM_INT_STATUS_TRANSFER_COMPLETE) == 0)
        && (--timeout != 0));
    DBG_TIMEOUT()
    if (timeout == 0)
    {
  ...
  if ((response_type == SDHOST_RESPONSE_R1) || (response_type == SDHOST_RESPONSE_R1b))
  {
#ifdef SD_HOST_DEBUG
  ...
  /* Check if any error occurred. Note that SDHOST_CMD_TIMEOUT (if any) has been detected and
...
#ifdef SD_HOST_DEBUG
    printf("\r\nerrStatus = ");
    printNum((uint32_t)errStatus, 'h', 1);
#endif /* SD_HOST_DEBUG */
    errStatus = 0;
...
/**
 *  @brief This function performs data transfers to/fr0m the SD card
 *
 *  This function follows paragraph 3.7.2.1 in the SD Host Controller Simplified Specification V3.00
 *  The flow has been modified so that each transfer type has it's own path through the function,
 *  instead of having common parts of the function as implied in the specification.
 *
 *  @param [in] direction - SDHOST_READ / SDHOST_WRITE
 *  @param [in] buf       - Data buffer pointer
 *  @param [in] num_bytes - Number of bytes to be transferred
...
SDHOST_STATUS sdhost_transfer_data(uint8_t direction, void *buf, uint32_t num_bytes, uint32_t addr)
{
   ...
   *        Option 2 is to add dummy data in the last block to fill the block size. This is
   *                 efficient but the card must manage or remove the dummy data.
   */
  ...
  /* Calculate how many "whole" blocks are needed. The remaining byte will be sent in one
  ...
  if (num_blocks == 1)
  {
    ...
    if (direction == SDHOST_WRITE)
    {
      ...
      cmd = (24 << 8) | CMD_TYPE_NORMAL | CMD_RESPONSE_R1 | CMD_DATA_PRESENT;
      transfer_mode = SDHOST_DMA_DISABLED | SDHOST_AUTO_CMD12_DISABLED | SDHOST_SINGLE_BLK |
                      SDHOST_WRITE;
      ...
      timeout = COMM_TIMEOUT_U32;
      while (((read_sdhost_reg(SDH_NRML_INT_STATUS) & NORM_INT_STATUS_BUF_W_RDY) == 0)
          && (--timeout != 0));
      DBG_TIMEOUT()
      if (timeout == 0)
      {
    ...
    else if (direction == SDHOST_READ)
    {
      ...
      /* (4) Set Transfer Mode Register and Command Register at the same time */
      cmd = (17 << 8) | CMD_TYPE_NORMAL | CMD_RESPONSE_R1 | CMD_DATA_PRESENT;
      transfer_mode = SDHOST_DMA_DISABLED | SDHOST_AUTO_CMD12_DISABLED | SDHOST_SINGLE_BLK |
                      SDHOST_READ;
      ...
      timeout = COMM_TIMEOUT_U32;
      while (((read_sdhost_reg(SDH_NRML_INT_STATUS) & NORM_INT_STATUS_BUF_R_RDY) == 0)
          && (--timeout != 0));
      DBG_TIMEOUT()
      if (timeout == 0)
      {
  ...
  else if (num_blocks >= 2)
  {
    ...
    if (direction == SDHOST_WRITE)
    {
      ...
      cmd = (25 << 8) | CMD_TYPE_NORMAL | CMD_RESPONSE_R1 | CMD_DATA_PRESENT;
      transfer_mode = SDHOST_DMA_DISABLED | SDHOST_BLK_COUNT_ENABLED | SDHOST_AUTO_CMD12_ENABLED |
                      SDHOST_MULTI_BLK | SDHOST_WRITE;
      SDHOST->TM_CMD = (cmd << 16) | transfer_mode;
    ...
    else if (direction == SDHOST_READ)
    {
      ...
      /* (4) Set Transfer Mode Register and (5) Command Register at the same time */
      cmd = (18 << 8) | CMD_TYPE_NORMAL | CMD_RESPONSE_R1 | CMD_DATA_PRESENT;
      transfer_mode = SDHOST_DMA_DISABLED | SDHOST_BLK_COUNT_ENABLED | SDHOST_AUTO_CMD12_ENABLED |
                      SDHOST_MULTI_BLK | SDHOST_READ;
      SDHOST->TM_CMD = (cmd << 16) | transfer_mode;
 ...
 *  @param [in] cmd_id - Command info
 *  @param [in] arg    - Command argument (32 bits)
 *  @param [in] pData  - Buffer pointer
 *  blk_size size of block to be read
 *  @returns  a SD Host Status
 */
static SDHOST_STATUS sdhost_send_wide_data_cmd(uint8_t cmd_id, uint32_t arg, uint8_t *pData,
                                               uint16_t blk_size)
{
  uint16_t cmd;
  uint16_t transfer_mode;
...
/** @brief This function read the contents of Extended CSD register by sending the eMMC CMD8 to
 *         record Device properties.
 *  @returns a SD Host Status
 */
static SDHOST_STATUS sdhost_chk_mmc_ext_csd(uint8_t *mmc_highspeed_support,
                                            uint32_t *mmc_sector_count)
{
  ...
  {
    /* For bitwise details of Extended CSD register, refer clause 7.4 of JEDEC e•MMC
       Standard v5.0 (JESD84-B50) */
...
bool sdhost_cmd6(void)
{
  ...
  /** Checks:
   * 1. CMD6 is only supported in cards with SD Spec v1.10 and above, so we need to check SCR or CSD
   *    register [bit 10 of CCC code]
   * 2. CMD6Mode 0 should be sent to check if the card supports the function first, before modifying
   *    the function via CMD6Mode 1
   * 3. Additionally the BUSY status in the response specifies whether a switch function can be
   *    issued now
   **/
  /* Send CMD16 to set the card's block length to 64 bytes, note that this is different from the
  ...
  sdhost_status = sdhost_send_wide_data_cmd(SD_MMC_CMD_SWITCH, CHECK_FUNCTION_HS_MODE, pBuff,
                                            SDCARD_SDSTATUS_BLK_SIZE);
  ...
  if (((pBuff[13] & 0x02) == 0x02) && ((pBuff[16] & 0x0f) == 0x01) && ((pBuff[30] & 0x02) == 0x00))
  {
    ...
    sdhost_status = sdhost_send_wide_data_cmd(SD_MMC_CMD_SWITCH, SET_FUNCTION_HS_MODE, pBuff,
                                              SDCARD_SDSTATUS_BLK_SIZE);
    ...
    if (((pBuff[13] & 0x02) == 0x02) && ((pBuff[16] & 0x0f) == 0x01))
    {
      timeout = 1000;
      while (timeout--)
        ; /* A small delay before changing parameters*/
      return true;
    }
    else
    {
...
static SDHOST_STATUS sdhost_mmc_startup(void)
{
  ...
  sdhost_status = sdhost_send_command(SD_MMC_CMD_ALL_SEND_CID, SDHOST_BUS_CMD,
                                      /* CMD2 responses with R2, which is the 127-bit CID */
                                      NOT_IN_USE, 0, sdhost_context.CID);
  ...
  sdhost_status = sdhost_send_command(SD_MMC_CMD_SEND_RELATIVE_ADDR, SDHOST_BUS_CMD, NOT_IN_USE,
                                      (sdhost_context.RCA << 16), &response);
  if (sdhost_status != SDHOST_OK)
  {
  ...
  if (UNSTUFF_BITS(sdhost_context.CSD, 122, 4) >= 0x04)
  {
    ...
    sdhost_status = sdhost_send_command(SD_MMC_CMD_SWITCH, SDHOST_APP_SPECIFIC_CMD, NOT_IN_USE,
                                        SDHOST_CMD6_ARG_BUS_WIDTH, &response);
    if (sdhost_status != SDHOST_OK)
    {
    ...
    sdhost_status = sdhost_send_command(SD_MMC_CMD_SWITCH, SDHOST_APP_SPECIFIC_CMD, NOT_IN_USE,
                                        SDHOST_CMD6_ARG_HS_TIMING, &response);
    if (sdhost_status != SDHOST_OK)
    {
  ...
  /** Bits [127,126] decodes CSD structure, bits [125:122] specification version in 128 bit wide CSD
   *  register
   ...
   */
  if ((UNSTUFF_BITS(sdhost_context.CSD, 126, 2) >= 0x02) &&
      (UNSTUFF_BITS(sdhost_context.CSD, 122, 4) >= 0x04))
  {
...
static SDHOST_STATUS sdhost_sd_startup(void)
{
  ...
  if ((flag8 == 0x01) || (flag8 = 0x00))
  {
    ...
    /**
     *  Note: If the voltage window field is set to zero, it is called "inquiry CMD41" that does not
     *        start initialization.
     *        It is used for getting OCR (OCR = response).
     *        The inquiry ACMD41 shall ignore the other field (bit 31-24) in the argument. So it is
     *        safe to use '0' for the argument.
     */
    ...
    else
    {
       ...
       *
       *  Note: If the voltage window field (bit 23-0) in the argument is set to non-zero at the
       *        first time, it is called "first ACMD41" that starts initialization.
       *
       *  The other field (bit 31-24) in the argument is effective. Since this host supports high
       *  capacity, HCS is set to 1.
    ...
    sdhost_status = sdhost_send_command(SD_MMC_CMD_SEL_DESEL_CARD, SDHOST_BUS_CMD, NOT_IN_USE,
                                        (sdhost_context.RCA << 16), &response);
...
/* Clear transfer complete interrupt flag due to CMD7 */
     ...
     *  NOTE: The SDHC "base clock" is 50MHz and NOT 100MHz, that's why we use a divide by 2 factor
     *        here.
     *  See redmine bug 402 - http://glared1.ftdi.local/issues/402
     */
    if (sdhost_cmd6())
 ...
 *  The numbers in parentheses indicate the progress of the steps shown in Para 6.1.3
 ...
 *  @param [in] response - Pointer to store the contents of SD Status register (64 bytes). Note that
...
void sdhost_get_card_status_reg(uint32_t *pBuff)
{
  ...
  /* (4) Set Transfer Mode Register and Command Register at the same time */
  cmd = (SD_MMC_CMD_SEND_STATUS << 8) | CMD_TYPE_NORMAL | CMD_RESPONSE_R1 | CMD_DATA_PRESENT;
  transfer_mode = SDHOST_DMA_DISABLED | SDHOST_AUTO_CMD12_DISABLED |
