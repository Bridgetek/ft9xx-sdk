/**
    ...
    @brief API for USB device stack DFU on FT900.
 ...
 *           A page is the smallest area of Flash that can be programmed. All programming operations
 *           must be aligned on a Flash page and be a multiple of the Flash page in length.
 */
...
/** @brief Countdown millisecond counter to return to DFUSTATE_APPIDLE from DFUSTATE_APPATTACH if a
 *         reset is not received within the time period specified in DFU_DETACH request. This is
 *         updated by the dfu_timer function. The application must call this function at millisecond
 *         intervals to ensure accurate timing.
 */
 ...
 *  @brief The amount of data received in a DNLOAD or UPLOAD operation is normally less than the
 *         page size required by the Flash. Therefore it is cached in the dfuFlashBuffer until either:
 *         - A complete page of data has been received.
 *         - A Zero-length packet is received indicating the end of data transfer.
 *         - A download to a different page is requested.
 *         If one of these happens then the buffer is committed to Flash. The buffer MUST be
 *         MEMORY_ALIGN_SIZE bit aligned. The smallest size of Flash which can be erased is a sector,
 *         this is several pages in size. Therefore when a page is written to in a different sector
 *         then that sector must be erased before it is programmed.
 */
//@{
...
void USBD_DFU_class_req_download(uint32_t address, uint16_t dataLength)
{
           ...
           ((address % FLASH_PAGE_SIZE) + dataLength <= FLASH_PAGE_SIZE))
  {
    ...
    if ((address + dataLength) <= FLASH_TOP_ADDRESS)
    {
      ...
      USBD_transfer_ep0(USBD_DIR_OUT, &dfuFlashBuffer[address % FLASH_PAGE_SIZE], dataLength,
                        dataLength);
...
void USBD_DFU_class_req_upload(uint32_t address, uint16_t dataLength)
{
  ...
  if ((address + dataLength) <= UPLOAD_FLASH_TOP_ADDRESS)
  {
        ...
        ((address % FLASH_PAGE_SIZE) + dataLength <= FLASH_PAGE_SIZE))
    {
      ...
      // Read in data from OUT endpoint.
      USBD_transfer_ep0(USBD_DIR_IN, &dfuFlashBuffer[address % FLASH_PAGE_SIZE], dataLength,
                        dataLength);
