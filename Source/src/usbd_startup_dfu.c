/**
  ...
  @brief Standalone function called from an application to initiate the USB device
 ...
 *  @brief Indication of how the USB device is powered and the size of the control endpoint Max
 *         Packets.
 */
//@{
...
#define USB_CONFIG_BMATTRIBUTES_VALUE (USB_CONFIG_BMATTRIBUTES_SELF_POWERED |    \
 ...
 *  @brief Determines the parts of the DFU specification which are supported by this firmware.
 ...
 *         USB_DFU_BMATTRIBUTES_WILLDETACH - Device perform a detach and attach sequence to change
 *                                           into DFU mode. If this is not supported then the host
 *                                           must reset the USB after a detach request. This will
 *                                           cause the device to reset and enter DFU mode.
 *         USB_DFU_BMATTRIBUTES_MANIFESTATIONTOLERANT - Not supported by the DFU library.
 */
//@{
 ...
 *         Leaving the allocation size blank will make an array exactly the size of the string
 *         allocation.
 */
//@{
 ...
 *  @brief Number of bytes in block, sent in each DFU_DNLOAD request from the DFU update program on
 *         the host. This is simplified in that the meaning of a block is an arbitrary number of
 *         bytes. This is intentionally a multiple of the maximum packet size for the control
 *         endpoints.
 *         It is used in the DFU functional descriptor as wTransferSize. The maximum size supported
 *         by the DFU library is 256 bytes which is the size of a page of Flash.
 */
//@{
 ...
 *  @brief Number of milliseconds to wait after DFU is started for no activity to reset the device.
...
static DESCRIPTOR_QUALIFIER uint8_t STARTUP_string_descriptor[STRING_DESCRIPTOR_ALLOCATION] =
{
  ...
  0x0a, USB_DESCRIPTOR_TYPE_STRING, 'F', 0x00, 'T', 0x00, 'D', 0x00, 'I', 0x00,
 ...
 *  @param [in] p - Parameters (machine dependent)
 ...
 *  @details Handle class requests from the host application. The bRequest value is parsed and the
 *           appropriate action or function is performed. Additional values from the
 *           USB_device_request structure are decoded and the DFU state machine and status updated
 *           as required.
 *  @param [in] req - USB_device_request structure containing the SETUP portion of the request from
 *                    the host.
 *  @return status - USBD_OK if successful or USBD_ERR_* if there is an error or the bRequest is not
 *                   supported.
 **/
static int8_t STARTUP_class_req_cb(USB_device_request *req)
{
      ...
      USB_BMREQUESTTYPE_RECIPIENT_INTERFACE)
  {
    ...
    case USB_CLASS_REQUEST_DNLOAD:
       ...
       */
      USBD_DFU_class_req_download(req->wValue * DFU_TRANSFER_SIZE, req->wLength);
      status = USBD_OK;
      break;
    ...
    case USB_CLASS_REQUEST_UPLOAD:
       ...
       * to program based on the size of the transfer size.
       */
      USBD_DFU_class_req_upload(req->wValue * DFU_TRANSFER_SIZE, req->wLength);
      status = USBD_OK;
      break;
 ...
 *  @details Handle standard GET_DESCRIPTOR requests from the host application.
 *           The hValue is parsed and the appropriate device, configuration or string descriptor is
 *           selected.
 *           For device and configuration descriptors the DFU state machine determines which
 *           descriptor to use. For string descriptors the lValue selects which string from the
 *           string_descriptors table to use. The string table is automatically traversed to find
 *           the correct string and the length is taken from the string descriptor.
 *  @param [in] req - USB_device_request structure containing the SETUP portion of the request from
 *                    the host.
 *  @return status - USBD_OK if successful or USBD_ERR_* if there is an error or the bmRequestType
 *                   is not supported.
 **/
static int8_t STARTUP_standard_req_get_descriptor(USB_device_request *req, uint8_t **buffer,
                                                  uint16_t *len)
{
  uint8_t *src = NULL;
 ...
 *           The bRequest value is parsed and the appropriate action or function is performed.
 *           Additional values from the USB_device_request structure are decoded and provided to
 *           other handlers. There are no vendor requests requiring handling in this firmware.
 *  @param [in] req - USB_device_request structure containing the SETUP portion of the request from
 *                    the host.
 *  @return status - USBD_OK if successful or USBD_ERR_* if there is an error or the bRequest is not
 *                   supported.
  ...
  if (req->bRequest == WCID_VENDOR_REQUEST_CODE)
  {
    if (req->wIndex == USB_MICROSOFT_WCID_FEATURE_WINDEX_COMPAT_ID)
    {
      if (req->bmRequestType & USB_BMREQUESTTYPE_DIR_DEV_TO_HOST)
      {
        ...
        USBD_transfer_ep0(USBD_DIR_IN, (uint8_t *)&wcid_feature_dfumode, length, length);
        // ACK packet
 ...
 *  @details Called when the USB bus enters the resume state prior to restating after a suspend.
 *  @param [in] status - unused.
 */
...
static void STARTUP_reset_cb(uint8_t status)
{
  ...
  // If we are at DFUSTATE_MANIFEST_WAIT_RESET stage and do not support detach then the DFU will
  // reset the chip and run the new firmware. From the DFUSTATE_APPIDLE state advance to
  // DFUSTATE_DFUIDLE. Other states will not advance the state machine or may move this to
  // DFUSTATE_DFUERROR if it is inappropriate to find a reset at that stage.
  USBD_DFU_reset();
 ...
 *  @details Called when a power management event occurs by the interrupt handler.
 *  @param[in] status - unused.
 */
...
static void STARTUP_DFU_go(uint16_t timeout)
{
  ...
  // Initialise the USB device with a control endpoint size of 8 to 64 bytes. This must match the
  // size for bMaxPacketSize0 defined in the device descriptor.
...
void startup_dfu(int timeout)
{
  ...
  timer_init(timer_select_a, 100, timer_direction_down, timer_prescaler_select_on,
