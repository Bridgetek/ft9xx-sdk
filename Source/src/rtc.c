...
int8_t rtc_start(void)
{
  ...
  else
  {
    ...
    // Status register is RW1C, clear everything
    write_reg(&RTC->RTC_STA, 0xFC);
  }
  return 0;
}
 ...
 *  @param [in] val - The value to set the Real Time Clock to
 *  @returns 0 on success, -1 otherwise
 */
 ...
 *  @param [in] val - A pointer to  store the current Real Time Clock Value into
 *  @returns 0 on success, -1 otherwise
 */
...
int8_t rtc_enable_interrupt(rtc_interrupt_t interrupt)
{
  ...
  {
    if (interrupt >= rtc_interrupt_max)
    {
      return -1;
    }
...
int8_t rtc_set_alarm(uint8_t number, struct tm *time, rtc_alarm_type_t alarm_type)
{
  ...
  if (sys_check_ft900_revB()) // 90x series rev B
  {
    if ((!number) || (number > 1) || (time == NULL) || (alarm_type != rtc_legacy_alarm))
    {
      return -1;
    }
  ...
  else
  {
    ...
    {
    case rtc_alarm_1Hz:
      t.Time |= (MASK_RTC_ALARM_MATCH_SECONDS | MASK_RTC_ALARM_MATCH_MINS |
                 MASK_RTC_ALARM_MATCH_HOURS | MASK_RTC_ALARM_MATCH_DAY_DATE);
      break;
    case rtc_alarm_match_sec:
      t.Time |= (MASK_RTC_ALARM_MATCH_MINS | MASK_RTC_ALARM_MATCH_HOURS |
                 MASK_RTC_ALARM_MATCH_DAY_DATE);
      break;
 ...
 *  @param [in] number - The type of option selected
 *  @param [in] time   - The value of the option selected
 *  @returns 0 on success, -1 otherwise
 */
...
/** @brief Converts a struct tm* to uint32_t to be written into the RTC
 *         The WRST bit should be set to update any of the registers that are buffered in the Alive
 *         register
 */
static uint32_t get_date(const struct tm *time)
{
 ...
 *  @param [in] val - Value to be written (uint32_t)
 */
static void write_reg(volatile uint32_t *reg, uint32_t val)
{
