...
{
  /** Noop added because i2cm driver was failing on -02 optimization (works with -O0)
  ...
  while (I2CM->I2CM_CNTL_STATUS & I2C_STATUS_BUSY) { /* Wait for the bus to be idle */ };
...
void i2cm_init(I2CM_speed_mode mode, uint32_t i2c_clk_speed)
{
  ...
  /** Set I2CM clock frequency by writing to I2CM_TIMER_PERIOD � Timer Period Register
   *  (address offset: 0x03).
   *  Frequency scaler (TIMER_PRD)is used in Standard/Fast/Fast-plus modes to calculate the I2C
   *  clock period (SCL_PRD).
   *
  ...
  if (mode == I2CM_HIGH_SPEED)
  {
    ...
    /* Then write timer value to the Normal register with the MSB set to 0 for
       standard/fast speed */
    I2CM->I2CM_TIME_PERIOD = (uint8_t)lTPVal;
    // Programming High speed for data transmission
    /* To write to the High-speed register, the MSB must be 1. FAST bit is cleared.
       Timer value is written*/
  ...
  else
  {
    ...
    {
      /* To write to the High-speed register, the MSB must be 1.
         You should write to it first to set the FAST bit. */
      I2CM->I2CM_TIME_PERIOD = MASK_I2CM_TIME_PERIOD_HS_SELECT | MASK_I2CM_TIME_PERIOD_FS_SELECT;
    }
    /* Then write timer value to the Normal register with the MSB set to 1 or 0 depending on high
       speed or not */
...
int8_t i2cm_write(const uint8_t addr, const uint8_t cmd, const uint8_t *data,
                  uint16_t number_to_write)
{
  ...
  if (ret == 0)
  {
    ...
    /* Check that the bus is not occupied by another master (this step is not needed in
       single-master system or in high speed mode since arbitration takes place during transmission
       of the master code) */
    if (isHighSpeed == 0)
    {
    ...
    while (number_to_read)
    {
      *data++ = I2CM->I2CM_DATA;
      ...
      if (number_to_read)
      {
        ...
        /* Write command to I2CMCR to read data byte. I2CM->I2CM_CNTL_STATUS = I2C_FLAGS_RUN; */
        if (i2c_wait_for())
        {
          ret = -1;
          break;
